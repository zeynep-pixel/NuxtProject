    ...
    <div style="padding-top: 300px;">
      <div v-if="isAuthenticated" class="user-info-container">
        ...
        <div v-if="userData" class="user-info" style="width:200px">
          <p style="font-family: Open Sans Condensed, sans-serif; font-weight: 700; font-size: 20px;">
            {{ userData.name }}
          </p>
          <p style="font-family: Open Sans Condensed, sans-serif; font-weight: 700; font-size: 16px;">
            {{ userData.address }}
          </p>
        </div>
...
const userData = ref<any>(null); // TypeScript'te daha güvenli bir şekilde referans tanımlama
...
onMounted(async () => {
  if (isAuthenticated) {
    try {
      ...
      console.error('Kullanıcı verileri alınırken hata oluştu:', error instanceof Error ? error.message : 'Bilinmeyen hata');
    }
  }
});
...
const handleLogout = async () => {
  try {
    ...
    console.error('Çıkış sırasında bir hata oluştu:', error instanceof Error ? error.message : 'Bilinmeyen hata');
  }
};
</script>
